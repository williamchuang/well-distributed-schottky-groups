...
def examples_of_10000(initial, increment):
    ...
    while counter < initial+10000*increment:
        ...
        if check_T_generate_a_Schottky(Lambda=counter,m=2):
            try:
                for i in range(13):
                    test=Hyperbolic_Distance_Gamma0(Lambda=counter,m=2,N=i)
                    mid=statistics.mean(Hyperbolic_Distance_Gamma0(Lambda=counter,m=2,N=i))
                    kk=1
        ...
        if len(out)>2:
            ...
            if flag==1:        
                useful_example+=1
                print("Useful:"+str(counter))
                print(out)
        
        counter+=increment
    print(useful_example)
examples_of_10000(0.3, 0.000001)
