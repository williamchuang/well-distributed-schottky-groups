...
#Check whether the group generated by T is a Schottky group:

return1=check_T_generate_a_Schottky(Lambda=0.3,m=2)

print(return1)

return2=Gamma0(Lambda=0.3,m=2,N=13)

print(return2)

return3=Hyperbolic_Distance_Gamma0(Lambda=0.3,m=2,N=13)

print(return3)

# Testing the number of nodes on the right of the average hyperbolic length 
# that is less than the average length is growing exponetially
# by checking the first reachable 13 levels.
import statistics
for i in range(13):
    test=Hyperbolic_Distance_Gamma0(Lambda=0.3,m=2,N=i)
    mid=statistics.mean(Hyperbolic_Distance_Gamma0(Lambda=0.3,m=2,N=i))
    first_occurence=1
    for node in test:
        if node > mid:
            print("N="+str(i))
            print("first_occurence="+str(first_occurence))
            break
        first_occurence+=1
